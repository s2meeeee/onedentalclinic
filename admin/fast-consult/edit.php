<?php
    ...
    include_once DAO_PATH.'/FastConsultDao.class.php';
    include_once DAO_PATH.'/FastConsultCommentDao.class.php';
    $backUrl = "/admin/fast-consult";
    ...
    $fastConsultCommentDao = new FastConsultCommentDao();

    $data = $dao->getDetail("fast_consults", $where);

    if (!$data) {
        alertLocationHref("게시글이 존재하지 않습니다.", $backUrl);
    }

    $fastConsultCommentsWhere = " WHERE fast_consult_id = ".$data['id'];
    $fastConsultCommentData = $fastConsultCommentDao->getList('fast_consult_comments', $fastConsultCommentsWhere);
?>
<section class="section">
    <div class="container-fluid">
        <?php echo sectionTitle("빠른상담 수정", "빠른상담 관리", "빠른상담 수정"); ?>
        <div class="col-lg-12">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="input-style-1">
                                <label>작성자</label>
                                <input type="text" name="title" title="작성자"
                                       placeholder="작성자를 입력해주세요" value="<?php echo $data['name'] ?>" readonly disabled>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="input-style-1">
                        ...
                        <div class="col-12 mt-4">
                            <div class="input-style-1">
                                <label>내용</label>
                                <textarea
                                        id="pain_area" name="pain_area" cols="30" rows="4" title="내용"
                                        placeholder="내용을 입력해주세요" readonly disabled><?php echo $data['pain_area'] ?></textarea>
                            </div>
                        </div>
                        </form>
                        <form id="comment_form" method="post">
                            <input type="hidden" name="mode" value="insert" readonly>
                            <input type="hidden" name="fast_consult_id" value="<?php echo $data['id'] ?>" readonly>
                        ...
                        <?php if($fastConsultCommentData['total_cnt'] > 0): foreach($fastConsultCommentData['values'] as $value): ?>
                            <div class="col-12">
                                <div class="input-style-1">
                                    <label>
                                        댓글
                        ...
                        <div class="col-12 d-flex justify-content-between">
                            <a class="btn info-btn btn-hover" href="/admin/fast-consult">목록</a>
                        </div>
                    </div>
...
<script>
    ...
    const updateComment = (id) => {
        ...
        jQuery.ajax({
            url:"../../controller/fast_consult_comment_proc.php",
            type:"post",
            dataType:"json",
            data,
            success: function(response) {
