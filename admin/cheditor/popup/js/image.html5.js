...
function makeThumbnail(image, fileType, randomName) {
    ...
    if (height > oEditor.config.thumbnailHeight) {
        height = oEditor.config.thumbnailHeight;
    }
    ...
    data.append('randomname', 'thumb_' + randomName);
    canvas = null;
...
DoUpload.prototype = {
    ...
    dataLoadHandler: function (evt) {
        ...
        if (evt.target.readyState === FileReader.DONE) {
            blob = new this.MyBlob(NewBlob(evt.target.result, filetype));
            image = new Image();
            ...
            image.onload = function () {
                var canvas = document.createElement("canvas"),
                    ...
                    xhr = new XMLHttpRequest(),
                    data = new FormData(),
                    fileFormat, el, imgBox = null, imgInfo = {}, bitmapData, randomName, ctx;

                xhr.open('POST', uploadScript, true);

                if (imageResizeWidth > 0 && width > imageResizeWidth) {
                    height = Math.ceil((imageResizeWidth / width) * height);
                    ...
                    bitmapData = canvas.toDataURL(filetype, oEditor.config.imgJpegQuality);
                    data.append('filehtml5', bitmapData.replace(/^(.*)base64,/g, ''));
                    canvas = null;
                } else {
                ...
                randomName = makeFilename() + fileFormat.toLowerCase();
                ...
                xhr.addEventListener("loadend", function onLoadendImageHandler(xhrevt) {
                    if (xhrevt.target.readyState === xhrevt.target.DONE) {
                        if (oEditor.config.makeThumbnail) {
                            makeThumbnail(image, filetype, randomName);
                            image.src = '';
                            image = null;
                        }
                    }
...
function init(dialog) {
    ...
    if (imageResizeWidth > 0) {
        ...
        imageResizeInput.setAttribute('placeholder', '원본');
        imageResizeInput.setAttribute('disabled', 'disabled');
    }
