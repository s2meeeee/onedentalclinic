...
(function () {
	...
	var disassemble = function (string, grouped) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (grouped)
				result.push(temp); //grouped가 설정된 경우 result에 temp 추가
				...
				if (step === 1) {
					...
					if (_isJung(cho)) {
						...
						if (
							...
							_isJung(
								(jung1 =
									array[complete_index + step + 1].charCodeAt(
										0
									))
							)
						) {
							//다음것이 있고 모음이면
							result.push(
								String.fromCharCode(_isJungJoinable(cho, jung1))
							); // cho와 jung1이 중성으로 조합가능한 경우 result에 조합한 문자 추가
