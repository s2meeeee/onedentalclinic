	...
	class NoticeDao
	{
        ...
        public function insert($params, $files, $folderName)
        {
            ...
            foreach ($fileUploadResult['files'] as $key => $file) {
                $params['notices'][$key.'_name'] = $file[0];
                $params['notices'][$key.'_path'] = $file[1];
            }

            if ($this->db->insert('notices',$params['notices'])) {
                $this->result = true;
        ...
        public function update($params, $files, $folderName, $where)
        {
            ...
            if ($this->db->update('notices',$params['notices'], $where)) {
                $this->result = true;
        ...
        {
            return $this->db->get_record("SELECT ".$column." FROM notices WHERE".$where);
        }
	}
?>