	...
	class PopupDao
	{
		...
		public function getList($view_table,$page=null,$limit=null,$where=null)
		{
            $query ="SELECT * FROM ".$view_table
					." WHERE 1 " . $where . " ORDER BY order_by asc, id DESC";
        ...
        public function insert($params, $files, $folderName)
        {
            ...
            foreach ($fileUploadResult['files'] as $key => $file) {
                $params['popups'][$key.'_name'] = $file[0];
                $params['popups'][$key.'_path'] = $file[1];
            }

            if ($this->db->insert('popups',$params['popups'])) {
                $this->result = true;
        ...
        public function update($params, $files, $folderName, $where)
        {
            ...
            if ($this->db->update('popups',$params['popups'], $where)) {
                $this->result = true;
        ...
        {
            return $this->db->get_record("SELECT ".$column." FROM popups WHERE".$where);
        }
	}
?>