	...
	class JobPosingDao
	{
        ...
        public function insert($params, $files, $folderName)
        {
            ...
            foreach ($fileUploadResult['files'] as $key => $file) {
                $params['job_postings'][$key.'_name'] = $file[0];
                $params['job_postings'][$key.'_path'] = $file[1];
            }

            if ($this->db->insert('job_postings',$params['job_postings'])) {
                $this->result = true;
        ...
        public function update($params, $files, $folderName, $where)
        {
            ...
            if ($this->db->update('job_postings',$params['job_postings'], $where)) {
                $this->result = true;
        ...
        {
            return $this->db->get_record("SELECT ".$column." FROM job_postings WHERE".$where);
        }
	}
?>