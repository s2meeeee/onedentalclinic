	...
	class EmployeeBoardDao
	{
        ...
        public function insert($params, $files, $folderName)
        {
            ...
            foreach ($fileUploadResult['files'] as $key => $file) {
                $params['employee_boards'][$key.'_name'] = $file[0];
                $params['employee_boards'][$key.'_path'] = $file[1];
            }

            if ($this->db->insert('employee_boards',$params['employee_boards'])) {
                $this->result = true;
        ...
        public function update($params, $files, $folderName, $where)
        {
            ...
            if ($this->db->update('employee_boards',$params['employee_boards'], $where)) {
                $this->result = true;
        ...
        {
            $filePaths = [];
            $values = $this->db->get_record("SELECT file_01_path, file_02_path, file_03_path, file_04_path, file_05_path, 
                                                file_06_path, file_07_path, file_08_path, file_09_path, file_10_path,
                                                file_11_path, file_12_path, file_13_path, file_14_path, file_15_path
                                                FROM employee_boards WHERE".$where);
        ...
        {
            return $this->db->get_record("SELECT ".$column." FROM employee_boards WHERE".$where);
        }
	}
?>