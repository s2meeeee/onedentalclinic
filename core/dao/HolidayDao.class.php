<?php
	class HolidayDao
	{
        ...
        public function getList($view_table,$page=null,$limit=null,$where=null,$calendar=false)
        {
            if ($calendar) {
                $where .= " AND  DATE(holiday) > CURRENT_DATE";
            }
        ...
        {
            if ($this->db->insert('holidays',$params['holidays'])) {
                $this->result = true;
        ...
        {
            if ($this->db->update('holidays',$params['holidays'], $where)) {
                $this->result = true;
