	...
	class TopBannerDao
	{
        ...
        public function insert($params, $files, $folderName)
        {
            ...
            foreach ($fileUploadResult['files'] as $key => $file) {
                $params['top_banner'][$key.'_name'] = $file[0];
                $params['top_banner'][$key.'_path'] = $file[1];
            }

            if ($this->db->insert('top_banner',$params['top_banner'])) {
                $this->result = true;
        ...
        public function update($params, $files, $folderName, $where)
        {
            ...
            if ($this->db->update('top_banner',$params['top_banner'], $where)) {
                $this->result = true;
        ...
        {
            return $this->db->get_record("SELECT ".$column." FROM top_banner WHERE".$where);
        }
	}
?>