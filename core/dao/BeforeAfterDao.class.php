<?php
	class BeforeAfterDao
	{
        ...
        public function getListMain($view_table,$limit)
        {
            $query ="SELECT id, file_02_path FROM ".$view_table. " ORDER BY id DESC";
        ...
        public function insert($params, $files, $folderName)
        {
            ...
            foreach ($fileUploadResult['files'] as $key => $file) {
                $params['before_afters'][$key.'_name'] = $file[0];
                $params['before_afters'][$key.'_path'] = $file[1];
            }

            if ($this->db->insert('before_afters',$params['before_afters'])) {
                $this->result = true;
        ...
        public function update($params, $files, $folderName, $where)
        {
            ...
            if ($this->db->update('before_afters',$params['before_afters'], $where)) {
                $this->result = true;
        ...
        private function getFilePaths($where)
        {
            ...
            $values = $this->db->get_record("SELECT file_01_path
                                                FROM before_afters WHERE".$where);
        ...
        {
            return $this->db->get_record("SELECT ".$column." FROM before_afters WHERE".$where);
        }
	}
?>