...
$result = array();
$files = array();
$folderName = "beforeAfter";
if($mode)
{
	include dirname(__FILE__).'/../core/class/MysqlDB.class.php';
	include dirname(__FILE__).'/../core/dao/BeforeAfterDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                if (strpos($key, 'input') === false) {
                    $params['before_afters'][$key] = $purifier->purify($value);
                }
            }
        ...
        case 'delete_file':
            $params['before_afters'][$_POST['fileName'].'_path'] = NULL;
            $params['before_afters'][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = "before_afters";
