...
$result = array();
$files = array();
$folderName = !empty($_POST['table_name']) ? $_POST['table_name'] : '';
if($mode)
{
	include dirname(__FILE__).'/../core/class/MysqlDB.class.php';
	include dirname(__FILE__).'/../core/dao/PreviewDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                if (strpos($key, 'input') === false) {
                    $params[$folderName][$key] = $purifier->purify($value);
                }
            }
        ...
        case 'delete_file':
            $params[$folderName][$_POST['fileName'].'_path'] = NULL;
            $params[$folderName][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = $folderName;
            $result =  $dao->delete_file($params, $_POST['fileName'], $where, $folderName);
        break;
