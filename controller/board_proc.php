...
if($mode)
{
	...
	include dirname(__FILE__).'/../core/dao/DefaultDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                if (strpos($key, 'input') === false) {
                    $params['board'][$key] = $purifier->purify($value);
                }
                if ($key == "types") {
                    $folderName = $value;
                }
        ...
        case 'delete_file':
            $params['board'][$_POST['fileName'].'_path'] = NULL;
            $params['board'][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = "board";
