...
if($mode)
{
	...
	include dirname(__FILE__).'/../core/dao/PostDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                if (strpos($key, 'input') === false) {
                    $params['posts'][$key] = $purifier->purify($value);
                }
            }
        ...
        case 'delete_file':
            $params['posts'][$_POST['fileName'].'_path'] = NULL;
            $params['posts'][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = "posts";
