...
$result = array();
$files = array();
$folderName = !empty($_POST['gubun']) ? $_POST['gubun'] : '';
if($mode)
{
	include dirname(__FILE__).'/../core/class/MysqlDB.class.php';
	include dirname(__FILE__).'/../core/dao/TopBannerDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                if (strpos($key, 'input') === false) {
                    $params['top_banner'][$key] = $purifier->purify($value);
                }
            }
        ...
        case 'delete_file':
            $params['top_banner'][$_POST['fileName'].'_path'] = NULL;
            $params['top_banner'][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = "top_banner";
