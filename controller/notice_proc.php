...
if($mode)
{
	...
	include dirname(__FILE__).'/../core/dao/NoticeDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                if (strpos($key, 'input') === false) {
                    $params['notices'][$key] = $purifier->purify($value);
                }
            }
        ...
        case 'delete_file':
            $params['notices'][$_POST['fileName'].'_path'] = NULL;
            $params['notices'][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = "notices";
