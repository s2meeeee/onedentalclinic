...
if($mode)
{
	...
	include dirname(__FILE__).'/../core/dao/StepDao.class.php';
	...
	switch($mode)
	{
		case 'insert' :
            ...
            foreach($_POST as $key => $value) {
                $params['steps'][$key] = $purifier->purify($value);
            }
        ...
        case 'delete_file':
            $params['steps'][$_POST['fileName'].'_path'] = NULL;
            $params['steps'][$_POST['fileName'].'_name'] = NULL;
            unset($_POST['mode']);
            $where = " id = ".$_POST['id'];
            unset($_POST['id']);
            $table = "steps";
        ...
        case 'get_steps':
            $result =  $dao->board->getDoctors('steps', $_POST['department_id']);
        break;
